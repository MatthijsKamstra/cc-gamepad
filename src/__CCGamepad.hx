...
class CCGamepad {
	...
	public function onButton(func:Function, ?arr:Array<Dynamic>) {
		var action:Action = new Action(func, arr, false);
	...
	public function onButtonOnce(id:Int, func:Function, ?arr:Array<Dynamic>) {
		...
		if (_options.onButtonArray == null)
			_options.onButtonArray = [];

		_options.onButtonArray.push(action);
	}
		...
		for (i in 0...gamepad.buttons.length) {
			...
			if (currentButton.pressed) {
				...
				switch (i) {
					...
					default:
						// if (_options.onButtonArray != null) {
						// 	for (i in 0..._options.onButtonArray.length) {
						// 		var action = _options.onButtonArray[i];

						// 		var _func = action.func;
						// 		var _arr = (action.arr != null) ? action.arr : [BUTTON_MAP.get(i)];

						// 		if (previousButtonID != i && action.id == i) {
						// 			Reflect.callMethod(_func, _func, _arr);
						// 		}
						// 	}
						// } else
						if (_options.onButton != null) {
							...
							if (_options.onButton.isOnce == false) {
								Reflect.callMethod(_func, _func, _arr);
							}
						}
				}
...
typedef SettingsObject = {
	...
	// buttons
	@:optional var onButtonArray:Array<Action>;
